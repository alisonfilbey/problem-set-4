...
def dynamic_scraper(year, month):
  ...
  while True:
    ...
    for i in range(len(span_tags)):
      ...
      #crawl through url and extract tags again 
      response_crawl = requests.get(link)
      soup_crawl = BeautifulSoup(response_crawl.text, 'lxml')
      span_crawl = soup_crawl.find("span", 
        string=lambda text: text and "Agency:" in text)
      
      #extract agency name from crawled tags
      if span_crawl:
        agency_name = span_crawl.find_next_sibling(string=True).strip()
        agency.append(agency_name)
      #add empty agency name if there is none
      else:
        agency_name = ""
        agency.append(agency_name)
...
There are 1,507 enforcement actions in the final dataframe. The earliest enforcement action scraped was from January 3, 2023, and the title, classification, agency, and url that was scraped for this enforcement action is listed above. 
